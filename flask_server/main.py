...
from flask_server.modules.database.sql_chain import SQLDatabaseChain
from flask_server.modules.database.sql_database import SQLDatabase
from flask_server.modules.agents.ReActMemoryAgent import ReActMemoryAgent
from flask_server.modules.api.api_chain import APIChain
from flask_server.modules.api.api import API
from flask_server.modules.resources.resource import Resource
from flask_server.modules.utils.text_formatting import colored_text_to_md
...
def get_chat_history(history):
    return "\n".join(
        [
            ("AI: " if entry["user"] == "bot" else "USER: ") + entry["text"]
...
def run(history, socket=None, capture_output=True):
    ...
    db_chain = SQLDatabaseChain(
        llm=OpenAI(temperature=0), database=db, verbose=True, debug=False
    )
...
def toy_test():
    ...
    history2 = [
        *history1,
        ...
        {"user": "user", "text": "Great can you rekick these models?"},
    ]
