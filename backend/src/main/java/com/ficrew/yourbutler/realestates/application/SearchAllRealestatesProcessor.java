...
public class SearchAllRealestatesProcessor {
    ...
    public Page<RealestateDocument> execute(SearchCommand searchCommand) {
        ...
        textQuery.filter(QueryBuilders.rangeQuery("dealOrWarrantPrc_numeric").gte(searchCommand.getDwMin()).lte(searchCommand.getDwMax()))
            .filter(QueryBuilders.rangeQuery("rentPrc_numeric").gte(searchCommand.getRpMin()).lte(searchCommand.getRpMax()))
            .filter(QueryBuilders.rangeQuery("maintenanceFee").gte(searchCommand.getMfMin()).lte(searchCommand.getMfMax()))
        ...
        Page<RealestateDocument> results = elasticsearchRealestateRepository.searchAllRealestate(searchCommand, pageable);
        ...
        SearchHits<RealestateDocument> searchHits = elasticsearchTemplate.search(searchQuery, RealestateDocument.class);

        // 6. 결과 반환
        return new SearchPage<>(searchHits, pageable);
    }
}
